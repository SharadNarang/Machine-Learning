...
a3 = sigmoid(z3);
h = a3';
#[max_value max_index] = max(a3_final, [], 2);
#h = max_index;
I = eye(max(y));
y01 = I(y,:);

#J = sum((1/m)*sum(-y01 .* log(h) - (1 - y01) .* log(1-h)));
...
J01 = sum((1/m)*sum(-y01 .* log(h) - (1 - y01) .* log(1-h)));
J02 = sum(sum(((lambda/(2.*m)).*(Theta1_reg.*Theta1_reg))));
J03 = sum(sum(((lambda/(2.*m)).*(Theta2_reg.*Theta2_reg))));
